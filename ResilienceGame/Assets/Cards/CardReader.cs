        ...
        for (int i = 1; i < allCSVObjects.Length; i++) {
            ...
            if (individualCSVObjects.Length > 1) // excel adds an empty line at the end
            {
                ...
                if (numberOfCards > 0) {
                    ...
                    foreach (string type in methods) {
                        switch (type) {
                            ...
                            case "TemporaryReductionOfTurnsLeft":
                                ...
                                break;
                            case "CancelTemporaryReductionOfTurns":
                                tempCard.ActionList.Add(new CancelTemporaryReductionOfTurns());
                                break;
                    ...
                    // 16:  Damage/Heal
                    if (int.TryParse(individualCSVObjects[16].Trim(), out int result)) {
                        tempCard.data.facilityAmount = result;
                    }
                    else {
                        tempCard.data.facilityAmount = 0;
                    }
